  ...
  <div class="app-container">
    ...
    <el-table v-loading.body="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="160px" align="center" label="修改时间">
        <template slot-scope="scope">
          <span>{{ new Date(scope.row.updatedTime) | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column width="160px" align="center" label="创建时间">
        <template slot-scope="scope">
          <span>{{ new Date(scope.row.createdTime) | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</span>
        </template>
      </el-table-column>
...
<script>
  ...
  import { queryGateway, addGateway, deleteGateway, updateGateway, refreshGateway } from '@/api/admin/gateway'
